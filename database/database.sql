...
CREATE DATABASE IF NOT EXISTS `event_v2` /*!40100 DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci */ /*!80016 DEFAULT ENCRYPTION='N' */;
USE `event_v2`;
...
-- Dumping data for table event_v2.attendees: ~2 rows (approximately)
...
CREATE TABLE IF NOT EXISTS `coordinator_applications` (
  ...
  `status` enum('pending','approved','rejected','reapplied') DEFAULT 'pending',
  `admin_notes` text,
...
CREATE TABLE IF NOT EXISTS `events` (
  ...
  `booking_phone` varchar(30) DEFAULT NULL,
...
-- Dumping data for table event_v2.events: ~1 rows (approximately)
...
-- Dumping data for table event_v2.orders: ~1 rows (approximately)
...
-- Dumping data for table event_v2.order_items: ~1 rows (approximately)
...
CREATE TABLE IF NOT EXISTS `users` (
  ...
  `role` enum('ordinary','admin','checker','coordinator') DEFAULT 'ordinary',
  `active` tinyint(1) DEFAULT '1',
  `email` varchar(100) DEFAULT NULL,
  `phone` varchar(20) DEFAULT NULL,
  `organization` varchar(150) DEFAULT NULL,
  `coordinator_status` enum('pending','approved','rejected') DEFAULT NULL,
